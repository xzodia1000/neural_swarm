...
def configure_pso_page(ann_main):
    ...
    optimize_activation = st.toggle("Optimize Activation", value=True)
    opt = st.radio("Optimization", options=["Minimize", "Maximize"])
    informants_size = st.number_input("Informants Size", min_value=1, value=6)
    ...
    gamma = st.number_input(
        "Gamma", help="Social Weight", min_value=0.0, value=0.0, step=0.1
    )
    delta = st.number_input(
        "Delta", help="Global Weight", min_value=0.0, value=0.0, step=0.1
    )
    ...
    if back:
        ...
        st.rerun()
    if next:
        tmp = beta + gamma + delta
        if tmp > 4 and tmp < 3 and not randomize_weights:
